...
io.on("connection", function (client) {
  ...
  client.on("joinGame", function (tank) {
    ...
    users[client.id] = tank.name;
    client.broadcast.emit("user-connected", tank.name);
  });
  ...
  client.on("leaveGame", function (tankId) {
    console.log(users[client.id] + " has left the game");
    game.removeTank(tankId);
    client.broadcast.emit("removeTank", tankId);
    //⁡⁣⁣⁢𝗪𝗵𝘆 𝗮𝗿𝗲 𝘄𝗲 𝗱𝗼𝗶𝗻𝗴 𝘁𝗵𝗶𝘀 𝘁𝘄𝗶𝗰𝗲?⁡
  ...
  client.on("send-chat-message", (message) => {
    ...
    console.log(users[client.id] + ": " + message);
  });
});
